...
- `tools/週會宣導/`：把角色對話腳本（JSON）轉成雙擊即播的單一 HTML「Podcast 對談」網頁，給早會/週會宣導用。多角色各自音調語速（瀏覽器內建 TTS）、字幕同步變色、簡易插畫隨劇情換表情、語速可調、播完停在金句+互動問題。每週新增一集只要加一個 `腳本/*.json` 再跑 `python3 產生網頁.py`，不用改網頁。播放引擎是技能 `.claude/skills/ai-dialogue-podcast-builder/`（可重複用在其他對話腳本專案）。目前已有《工程師的價值》系列第一～八篇（第八篇尚未開放到入口網站）；第六篇起改成短句口語、開場先拋反差的年輕化語氣（README 裡有說明），之後新集數都照這個語氣寫。入口網站在 `tools/週會宣導/index.html`，靠 GitHub Pages 自動部署，還有 Cloudflare 雲端回應收集／QR Code／即時文字雲（詳見 `tools/週會宣導/README.md`）。結尾畫面現在會唸金句、可以打字回應主管互動問題（純本機存檔，不連網不上傳，因應公司資安政策不能用 API／上傳檔案），另有 `文字雲.html` 把彙整好的回應統計成文字雲。
