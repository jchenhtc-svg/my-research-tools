...
五種使用方式：CLI（自己分析用，**現在會順便產生一份單一HTML報告，雙擊用瀏覽器開，不需要伺服器**）、網頁版（雲端部署給選手/助理教練用）、現地Docker版（單機起服務，同WiFi多裝置連線）、現地單機.exe版（單一電腦用，不需要Docker/Python，雙擊就跑）。
...
- `選手影片_analyzed_report.html`：**同一份報告的網頁版**，雙擊直接用瀏覽器打開，排版跟網頁版App一樣（分數卡片、紅色警示、行動計畫），完全不需要Flask/Node/Docker，也可以直接傳給選手/助理教練看——對方連Python都不用裝

其他選項：
```bash
python main.py 選手影片.mp4 --report-only   # 只出文字報告，不輸出標註影片（較快）
python main.py 選手影片.mp4 --no-html       # 不要HTML報告，只要txt
python main.py 選手影片.mp4 --open          # 分析完自動用瀏覽器打開HTML報告
...
想最快看到「網頁效果」的話，只需要跑：
```bash
python main.py 選手影片.mp4 --report-only --open
```
不用裝Docker、不用裝Node.js，幾秒鐘後瀏覽器就會自動打開分析報告。
...
已知限制：**偵測率明顯偏低（例如低於70%）的分析結果不該直接採信**——通常代表影片太短、選手中途出鏡、或動作剛好落在偵測空檔，樣本數不足會讓角度/划頻統計失真（實測時8/10分的一支影片偵測率只有48.3%，只抓到2次划水，可信度低於另外兩支96%+偵測率、判定4/10的影片）。✅ 已在HTML報告加上低偵測率警示橫幅（`src/html_report.py`，門檻70%），txt報告目前還沒有這個警示。
