...
8. **"Enrich the visuals" doesn't have to mean touching the character.** When a user wants more manga-style life on screen but explicitly says not to change the character art, add a *separate* reaction-mark layer instead of editing `faceSVG()`'s per-expression `extra` doodles: a small `EXPR_MARK`-keyed symbol (sweat drop / "!?" burst / anger cross-lines / lightbulb, etc.) in its own `#reactionMark` element, positioned near the head, that pops in via a CSS keyframe animation replaying automatically each time you swap in a fresh `<svg>` (no JS reflow hack needed — a newly-inserted element already plays its `animation` from frame 0). Keep it a sibling of `charWrap` inside `.stage`, never a child of `charWrap` — `showEndCard()` wipes `charWrap.innerHTML`, and per pitfall #1 anything nested inside would be silently orphaned on the next render. Only map a mark to a curated subset of expressions (the ones that are genuinely a "beat"); giving every line a mark defeats the purpose and clutters the panel.
9. **A boolean state flag plus a delayed `setTimeout` is a race condition waiting to happen — use a sequence token.** `onEndCard` needs to be true exactly while the end card is showing, but `flashRender()`'s `render()` call is itself delayed 110ms (the shutter-flash transition) and unconditionally sets `onEndCard=false` at the top of `render()`. If a click that advances to the last line schedules that delayed `render()`, and a *second* click (reaching the actual end) schedules `showEndCard()` in roughly the same 110ms window, the two timers can fire out of order: the stale `render()` call fires after `showEndCard()` already ran, silently flips `onEndCard` back to `false`, and the next click re-triggers the whole end-card transition again — duplicate TTS, duplicate everything. Rapid clicking (or a fast automated test) surfaces this; normal manual clicking mostly doesn't, which is exactly why it's easy to ship unnoticed. Fix: keep a single incrementing `actionSeq` counter, capture it (`const seq = ++actionSeq`) at the moment *any* delayed transition is scheduled (`flashRender()`, the autoplay-finished branch in `playFrom()`, the last-line branch in `next()`), and have the `setTimeout` callback check `seq === actionSeq` before acting. Any newer action bumps the counter and silently invalidates every older pending timer, so only the most recent transition is ever allowed to run.
10. **Anything read aloud has to survive being HTML.** `GOLDEN_QUOTE` is authored with `<br>` for line breaks in the visual quote card; feeding that string straight into `SpeechSynthesisUtterance` speaks the literal tag or, if you strip `<br>` by concatenating punctuation, can double it up (a quote already ending in `，` becomes `，，` once `<br>` itself is replaced with another `，`). Strip all tags first, then normalize (e.g. collapse `punctuation + break` down to the punctuation alone) — always spot-check the final spoken string, not just that no error was thrown.
11. **A local-only response box (no server) is a real, legitimate pattern — don't default to "needs a backend."** When a deployment can't call any network API (corporate policy, no accounts, no hosting) but still wants "collect responses from many viewers," the answer isn't to give up on collection — it's `Blob` + `<a download>`: each viewer's answer saves as its own small file to their own Downloads folder, with zero network calls. Aggregation then happens outside the page entirely (point everyone's browser download folder at a shared drive, or gather files by whatever means the organization already uses) — the page never needs to know how the files get collected. A companion page can then load a batch of those files via a plain `<input type="file" multiple>` (or drag-and-drop) and do all analysis client-side. For Chinese-text frequency analysis with no dictionary/segmentation service available offline, character bigrams (overlapping 2-character windows) after stripping whitespace/punctuation is a serviceable, honest substitute for real word segmentation — say so in the UI rather than presenting it as accurate tokenization. And when a page has a dropdown whose own options are derived from the loaded data (e.g. "filter by episode"), never rebuild those options from inside the same change-driven render function the dropdown's `change` event triggers — regenerating `<select>` innerHTML resets its selected value, so the filter appears to silently do nothing the instant it's changed. Split "rebuild options" (call only when the underlying dataset changes) from "re-render using the current selection" (call on every filter change).
